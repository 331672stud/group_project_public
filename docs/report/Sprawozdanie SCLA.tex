...
\hypersetup{
    ...
    urlcolor=accent,
    citecolor=primary,
    pdftitle={SCLA\_ML\_vuln\_detector --- Raport Techniczny},
    pdfauthor={Projekt SCLA\_ML},
    pdfsubject={Dostrajanie modelu LLM do wykrywania podatnosci w cyberbezpieczenstwie}
}
...
\begin{tcolorbox}
\begin{verbatim}
ollama create SCLA_ML_vuln_detector -f Modelfil
\end{verbatim}    
\end{tcolorbox}
...
\item \textbf{[Łatwy]} Atakujący wpisuje w pole hasła wartość \texttt{haslo' OR '1'='1}. Zakładając, że oryginalne zapytanie ma
postać:
\begin{tcolorbox}
...
\begin{itemize}
    ...
    \item hashuje hasło używając \texttt{hashlib.pbkdf2\_hmac}
            z losową solą (salt) --- sola musi być przechowywana
            razem z hashem;
...
(najczęściej JavaScript) do stron internetowych, przez co \emph{osadzając złośliwą treść w stronach}, które następnie są 
...
Podczas realizacji projektu najbardziej znacznym aspektem, przez który praca zespołowa kulała, był brak komunikacji w zespole. Mimo wstępnych prób oraz uzgodnienia ogólnego zarysu projektu, po pewnym czasie członkowie zespołu zatrzymali komunikację między sobą - projekt był w stagnacji do późnego terminu. 

Problem ten skutkował licznymi momentami dezinformacji, brakiem odpowiedniego przydzielenia zadań, niedopracowanymi oraz niedogadanymi sprawami, które mogły być załatwione trywialną rozmową, oraz zgrzytami między członkami zespołu. Nie sposób nie zauważyć, że problemy komunikacji członków oraz braku regularnych spotkań statusowych zahamowały pracę nad projektem.

Na szczęście, gdy przyszło co do czego, członkowie zespołu byli w stanie wykonać wstępnie rozdzielone aspekty projektu tak, aby końcowa aplikacja spełniała swoją podstawową funkcjonalność oraz założenia uzgodnione na początku procesu tworzenia. Niemniej jednak, stanowi to bardzo cenną nauczkę dla wszystkich członków zespołu - w celu odpowiedniego ukończenia projektu zespołowego kluczowa jest otwarta komunikacja, stworzenie przestrzeni gdzie członkowie chcą chętnie wyrażać swoje opinie oraz zaufanie w zespole.
